...
export default function ServiceCall(){
  ...
  const submit = (e)=>{
    e.preventDefault()
    localStorage.setItem('iw_service_request', JSON.stringify(form))
    setSubmitted(true)
  }
  return (
    <>
        ...
        <div className="card">
          {!submitted ? (
            <form onSubmit={submit}>
              ...
              <label>Notes</label>
              <textarea rows="4" value={form.notes} onChange={e=>setForm({...form, notes:e.target.value})} placeholder="Describe the issue" />
              <div style={{height:12}} />
              <button className="btn" type="submit">Submit Request</button>
            </form>
          ) : (
            <p>Thanks! Your request was recorded. We’ll email you a confirmation shortly.</p>
          )}
        </div>
      </div>
    </>
